...
type         : Integer { $$ = buildIntTree(Integer, 1); }
             | RealT   { $$ = buildIntTree(RealT  , 2); }
             | Boolean { $$ = buildIntTree(Boolean, 3); }
...
state_decls  : { $$ = NULL; }
             | statement SColon pStateDeclSeq   { $$ = $1; $$->next = $3; }
             | declaration SColon pStateDeclSeq { $$ = $1; $$->next = $3; }
             ;

...
pStateDeclSeq: { $$ = NULL; }
             ...
             | Var idlist TAssign type SColon pStateDeclSeq
                        { $$ = new DataAssignE( buildTree(TAssign, $2, $4, NULL) ); $$->next=$6; }
             ;
...
statement    : ref Assign expr
                        { $$ = new Assignment( buildTree(Assign, $1, $3, NULL) ); }
             | Assert bool_expr
                        { $$ = buildTree(Assert, $2, NULL, NULL); }
             | Begin state_decls End Loop
                        { $$ = new LoopE( buildTree(Begin, $2, NULL, NULL) ); }
             | Loop state_decls End Loop
                        { $$ = new LoopE(buildTree(Loop, $2, NULL, NULL)); }
             | Exit 
...
rel_expr     : sum 
             | rel_expr Eq sum 
                        { $$ = new EqE( buildTree(Eq, $1, $3, NULL) ); }
             | rel_expr Ne sum
                        { $$ = new NeE( buildTree(Ne, $1, $3, NULL) ); }
             | rel_expr Lt sum
                        { $$ = new LtE( buildTree(Lt, $1, $3, NULL) ); }
             | rel_expr Le sum
                        { $$ = new LeE( buildTree(Le, $1, $3, NULL) ); }
             | rel_expr Gt sum 
                        { $$ = new GtE( buildTree(Gt, $1, $3, NULL) ); }
             | rel_expr Ge sum
                        { $$ = new GeE( buildTree(Ge, $1, $3, NULL) ); }
             ;
...
sum          : prod 
             | sum Plus prod
                        { $$ = new PlusE( buildTree(Plus, $1, $3, NULL) ); }
             | sum Minus prod
                        { $$ = new SubE( buildTree(Minus, $1, $3, NULL) ); }
             ;

prod         : factor
             | prod Star factor
                        { $$ = new MulE( buildTree(Star, $1, $3, NULL) ); }
             | prod Slash factor
                        { $$ = new DivE( buildTree(Slash, $1, $3, NULL) ); }
             | prod Div factor
                        { $$ = new DivE( buildTree(Div, $1, $3, NULL) ); }
             | prod Mod factor
                        { $$ = new ModE( buildTree(Mod, $1, $3, NULL) ); }
             ;
...
basic        : ref
             ...
             | IntConst           
                  { $$ = new IntConstE (buildIntTree(IntConst, intConstStack.top()) );
                                    intConstStack.pop(); }
             | Real               
                  { $$ = new RealConstE( buildDoubleTree(Real, realConstStack.top()));
                                    realConstStack.pop(); }
             ;
%%
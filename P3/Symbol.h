...
private:
    ...
    {
        if      (op->kind == Assign || isRelational(op->kind))    { evalOperation(op); }
    }

    void evalOperation(tree &op)
    {
        Symbol * LHSymbol = currentTable->rlookup(op->first->name);
        string   RHSType  = evalType(op->second);
        //string   RHSType  = isRelational(op->kind) ? evalCalc(op) : evalType(op->second);
